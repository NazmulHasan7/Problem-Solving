...
to tell the gender of a user at the first glance. Last year, our hero went to a forum and had a 
nice chat with a beauty (he thought so). After that they talked very often and eventually they 
became a couple in the network.
But yesterday, he came to see "her" in the real world and found out "she" is actually a very 
strong man! Our hero is very sad and he is too tired to love again now. So he came up with a way
to recognize users' genders by their user names.
...
bool isVisited (string disStr, char ch){
...
int main()
{
    ...
    for(int i=0; i<str.length(); i++){
        if(!isVisited(disStr, str[i])){
            disStr += str[i];
        }
    }
    if(disStr.length() % 2 != 0)
